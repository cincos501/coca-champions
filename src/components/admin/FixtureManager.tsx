...
import type { Equipo, Partido, Grupo, DiaJuego } from '../../types/futbol.types';
...
export default function FixtureManager({ equipos, mostrarFeedback }: FixtureManagerProps) {
  ...
  // ⚡ NUEVO: Estado para controlar la jornada de juego activa en el gestor
  const [diaActivo, setDiaActivo] = useState<DiaJuego>('Sábado');
  ...
  // ⚡ FILTROS DINÁMICOS EN BASE AL DÍA SELECCIONADO EN LAS PESTAÑAS
  const equiposFiltradosPorDia = equipos.filter(e => e.dia_juego === diaActivo);
  const equiposDelGrupoFormulario = equiposFiltradosPorDia.filter(e => e.grupo === grupoSeleccionado);
  ...
  const handleProgramarPartido = async (e: React.FormEvent) => {
    ...
    if (idLocal === idVisitante) {
      mostrarFeedback('error', 'Un equipo no puede jugar contra si mismo.');
      return;
    }
    ...
    try {
      ...
      await FutbolService.programarPartido({
        dia_juego: diaActivo, // ⚡ Se guarda con el tag del dia de juego activo en la pestaña
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* ⚡ PESTAÑAS DE CONTROL DE JORNADA: Permite alternar la visualización completa */}
      <div className="flex bg-neutral-900 p-1.5 m-auto rounded-2xl border border-neutral-800 max-w-xs md:mx-0">
        <button
          type="button"
          onClick={() => { setDiaActivo('Sábado'); setIdLocal(''); setIdVisitante(''); }}
          className={`flex-1 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
            diaActivo === 'Sábado' ? 'bg-[#F40009] text-white shadow-lg' : 'text-gray-400 hover:text-white'
          }`}
        >
          Jornada Sábado
        </button>
        <button
          type="button"
          onClick={() => { setDiaActivo('Domingo'); setIdLocal(''); setIdVisitante(''); }}
          className={`flex-1 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
            diaActivo === 'Domingo' ? 'bg-[#F40009] text-white shadow-lg' : 'text-gray-400 hover:text-white'
          }`}
        >
          Jornada Domingo
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          {/* Bloque 1: Clasificación de Grupos Dinámica */}
          <div className="bg-white rounded-3xl p-5 shadow-xl border border-gray-100">
            <h3 className="font-black text-xs uppercase tracking-tight text-gray-900 mb-1 flex items-center gap-2">
              <Users className="w-4 h-4 text-[#F40009]" /> Distribución de Grupos ({diaActivo})
            </h3>
            <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider mb-3">
              Mapea los grupos para el torneo del {diaActivo.toLowerCase()}
            </p>
            
            {equiposFiltradosPorDia.length === 0 ? (
              <p className="text-[11px] text-gray-400 italic py-6 text-center">
                No hay equipos registrados para el {diaActivo}.
              </p>
            ) : (
              <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                {equiposFiltradosPorDia.map(eq => (
          ...
          <div className="bg-white rounded-3xl p-5 shadow-xl border border-gray-100">
            <h3 className="font-black text-xs uppercase tracking-tight text-gray-900 mb-4 flex items-center gap-2">
              <Calendar className="w-4 h-4 text-[#F40009]" /> Programador ({diaActivo})
            </h3>
            ...
            <form onSubmit={handleProgramarPartido} className="space-y-4">
              ...
              <button type="submit" disabled={guardando || equiposDelGrupoFormulario.length < 2} className="w-full bg-[#F40009] hover:bg-red-700 text-white py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all cursor-pointer disabled:opacity-40">
                Agendar Partido {diaActivo === 'Sábado' ? 'Sábado' : 'Domingo'}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl shadow-xl border border-gray-100 p-5">
          <h3 className="font-black text-sm uppercase tracking-tight text-gray-900 border-b border-gray-100 pb-3 flex items-center gap-2">
            <Trophy className="w-4 h-4 text-black" /> Calendario y Resultados ({diaActivo})
          </h3>

          <div className="space-y-6 max-h-132 overflow-y-auto pr-1 mt-4">
            {(['1RA VUELTA', '2DA VUELTA', '3RA VUELTA'] as const).map(vuel => {
              // ⚡ FILTRO CRÍTICO: Muestra solo los partidos agendados para el día activo de la pestaña
              const partidosDeVuelta = partidos.filter(p => p.dia_juego === diaActivo && p.vuelta === vuel);
              ...
              return (
                <div key={vuel} className="space-y-2">
                  ...
                  {partidosDeVuelta.length === 0 ? (
                    <p className="text-[11px] text-gray-400 italic pl-1 py-1">No hay partidos programados para el {diaActivo.toLowerCase()} en esta vuelta.</p>
                  ) : (
